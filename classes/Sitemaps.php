...
class Sitemaps {
    ...
    function auto_detect($excluded = array('sitemap')) {
        ...
        $files = get_filenames('SamenCms/controllers');
        ...
        for ($index = 0; $index < count($files); $index++) {
            ...
            try {
                include('SamenCms/controllers/' . $files[$index]);
            } catch (Exception $exc) {
    ...
    function build($file_name = null, $gzip = NULL) {
        $image_category_ids = $this->CI->config->item('image_categories');
        $video_categories = $this->CI->config->item('video_categories');
        $news_categories = $this->CI->config->item('news_categories');

        $map = $this->CI->config->item('sitemaps_header');

        if (!empty($image_category_ids))
        ...
        foreach ($this->items as $item) {
            ...
            if (in_array($item['category_id'], $image_category_ids)) {
                $map .= "\t<image:image>\n\t\t
